...
class PrincipalAccessMixin(UserPassesTestMixin):
    def test_func(self):
        return self.request.user.groups.filter(name='Principal').exists() or self.request.user.is_superuser
    ...
    def get_context_data(self, **kwargs):
        ...
        if status:
            if status in ['ENROLLED', 'TRANSFERRED', 'DROPPED']:
                ...
                students = students.filter(
                    academic_records__remarks='PASSED'
                )
                if school_year:
                    students = students.filter(academic_records__school_year=school_year)
...
class ExportReportCSVView(LoginRequiredMixin, PrincipalAccessMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        if grade_level:
            students = students.filter(academic_records__grade_level=grade_level).distinct()
        if school_year:
            students = students.filter(academic_records__school_year=school_year).distinct()
        if status and status in ['ENROLLED', 'TRANSFERRED', 'DROPPED']:
             students = students.filter(status=status)

        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="student_report_{datetime.date.today()}.csv"'

        writer = csv.writer(response)
        writer.writerow(['LRN', 'Last Name', 'First Name', 'Sex', 'Birthdate', 'Status', 'Address'])
...
class ExportReportPDFView(LoginRequiredMixin, PrincipalAccessMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        html_string = render_to_string('reports/report_pdf_template.html', {
            'students': students,
        ...
        if HTML:
            ...
            response['Content-Disposition'] = f'inline; filename="report_{datetime.date.today()}.pdf"'
...
class AnalyticsDashboardView(LoginRequiredMixin, PrincipalAccessMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        by_barangay = students.values('barangay').annotate(count=Count('lrn')).order_by('-count')
        by_city = students.values('city').annotate(count=Count('lrn')).order_by('-count')
        by_province = students.values('province').annotate(count=Count('lrn')).order_by('-count')
