    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            if isinstance(field.widget, forms.CheckboxInput):
                field.widget.attrs['class'] = 'h-4 w-4 text-deped-blue focus:ring-deped-blue border-gray-300 rounded'
            else:
                field.widget.attrs['class'] = 'mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-deped-blue focus:border-deped-blue sm:text-sm'
